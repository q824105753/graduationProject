...
<div class="page-content">
	<div class="content">
		<!-- 右侧内容框架，更改从这里开始 -->
		<form class="layui-form xbs" action="/bidding/admin/selectAllCompanyByPage" method="post">
			<div class="layui-form-pane" style="text-align: center;">
				<div class="layui-form-item" style="display: inline-block;">
					<label class="layui-form-label xbs768">查找</label>
					
					<div class="layui-input-inline">
						<input type="text" name="companyname" value="${companyname }" placeholder="请输入公司名"
							autocomplete="off" class="layui-input">
					</div>
					<div class="layui-input-inline" style="width: 80px">
						<i class="layui-icon"><input type="submit" class="layui-btn" lay-filter="add" value="&#xe615;"></i>
					</div>
				</div>
			</div>
		</form>
		...
		<button class="layui-btn"
			onclick="window.location.href='/bidding/admin/companyAdd'">
			<i class="layui-icon">&#xe608;</i>添加
		</button>
		<span class="x-right" style="line-height: 40px">共有数据：${companyCount } 条</span></xblock>
		<table class="layui-table">
			...
			<tbody>
				<c:forEach items="${clist.list }" var="c">
					<tr>
						<td><input type="checkbox" value="1" name=""></td>
						<td>${c.companyid }</td>
						<td>${c.companyname }</td>
						<td>${c.address.addressname }</td>
						<td>${c.tel }</td>
						<td>${c.user.name }</td>
						<td class="td-manage">
							...
							onclick="window.location.href='/bidding/admin/loadCompany?companyid=${c.companyid }'"
							...
							onclick="window.location.href='/bidding/admin/delCompany?companyid=${c.companyid }'" style="text-decoration: none">
		...
		<div class="x-right">
		<c:forEach items="${clist.pageView }" var="p">
			${p }
		</c:forEach>
		</div>
